...
function parsePage(html) {
    ...
    $('.brkts-match').each((_, el) => {
        ...
        // status:
        //   finished     -> 任一边有 .brkts-opponent-win 类
        //   in_progress  -> 没有 winner, 但任一边比分 > 0 (Liquipedia 已经开始填实时比分)
        //   upcoming     -> 其余
        let status = 'upcoming';
        if ($el.find('.brkts-opponent-win').length > 0) {
            status = 'finished';
        } else {
            const hasLiveScore = (score1 != null && score1 > 0) || (score2 != null && score2 > 0);
            // 兜底: Liquipedia 偶尔会用 .brkts-match-info-live 或 ongoing 标签
            const hasLiveBadge = $el.find('.brkts-match-info-live, .timer-object-countdown-live').length > 0;
            if (hasLiveScore || hasLiveBadge) status = 'in_progress';
        }
