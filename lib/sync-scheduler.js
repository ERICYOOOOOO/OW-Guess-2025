...
async function highFreqTick() {
    ...
    try {
        ...
        for (const m of matches) {
            ...
            // 触发自动 settle: 比赛非 finished, scraped 报 finished 且比分非零
            if (m.status !== 'finished' && s.status === 'finished'
                ...
                try {
                    ...
                    continue; // 已 settle, 跳过 lock 检查
                } catch (err) {
                    console.error(`[sync] auto-settle ${m.customId} 失败:`, err.message);
                }
            }

            // 触发自动 lock: 比赛 upcoming, scraped 报 in_progress
            if (m.status === 'upcoming' && s.status === 'in_progress') {
                try {
                    await Match.updateOne({ _id: m._id, status: 'upcoming' }, { $set: { status: 'locked' } });
                    autoLocked.push(m.customId);
                } catch (err) {
                    console.error(`[sync] auto-lock ${m.customId} 失败:`, err.message);
                }
            }
        }
